...
class Equipment
{
   ...
   // Swap method that accepts an integer value
   public void swap(int _equipID) throws DLException
   {
      // Try and catch statement
      try
      {
         ...
         // Call the start transaction method
         if(sql.startTrans())
         { 
            String holdName = this.getEquipmentName() + "";
            
            String query = "SELECT * FROM equipment WHERE equipID = ?;";
            ArrayList<String> al = new ArrayList<String>();
            al.add( _equipID + "");
            ArrayList<ArrayList<String>> myTable = sql.getData(query, al);
            ArrayList<String> list = myTable.get(0);
            String newHoldName = list.get(1);
            setEquipmentName(newHoldName);
            
            String query2 = "UPDATE equipment SET EquipmentName = ? WHERE EquipID = ?";
            ArrayList<String> al2 = new ArrayList<String>();
            al2.add(holdName);
            al2.add(_equipID + "");
            setNum(sql.setData(query2, al2));
            
            ArrayList<String> al3 = new ArrayList<String>();
            al3.add(newHoldName);
            al3.add(getEquipmentID() + "");
            setNum(sql.setData(query2, al3));
      ...
      {
         sql.closeConnect();
         errorMessage.add("java.Equipment: Swap: "+  e.getMessage());
         throw new DLException(e,errorMessage);  
      }
   }
